...
```{r detect_hist, echo=FALSE, message=FALSE, warning=FALSE}
#might need to consider fish going dowsntream
detect_hist <- df %>%
...
wb <- loadWorkbook("./data/detect_hist.xlsx")
sheets <- getSheets(wb)
# autosize column widths
autoSizeColumn(sheets[[1]], colIndex=1:ncol(df))
saveWorkbook(wb,"./data/detect_hist.xlsx")
...
AtWeir<-detect_hist%>%filter(str_detect(TagPath,"IR3|IR4|IR5|IML|IMNAHW"))
ggplot(AtWeir,aes(x=as.Date(LastObs,"%m/%d/%Y",tz="UTC"),fill=TagStatus))+
  geom_histogram(colour = 'black', alpha = .5, binwidth=1)+
  facet_grid(.~Mark.Species,scales="free")+
  scale_fill_brewer(palette = 'Set1') +
  theme_bw() +
  theme(legend.position = 'bottom') +
  theme(panel.grid.major=element_blank())+
  ggtitle("Tag status for detections upstream of IR3")+
  labs(x = "Last Observation Date at IR3/IR4/IML/IR5/IMNAHW",
       y = "Count")
