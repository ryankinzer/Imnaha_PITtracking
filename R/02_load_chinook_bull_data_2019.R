...
chs_url <- 'ftp://ftp.ptagis.org/MicroStrategyExport/feldhauj/2019_Imnaha_CompleteTagHistory.csv'#Chinook file
...
install_date <- ymd_hm("2019/06/01 15:00") # we could add time and second if we wanted
bull_cutoff_date <- ymd("20190301")
...
chs_bull_tags <- chs_bull_obs_raw %>% 
  distinct(`Tag Code`) %>%
  mutate(TagID = `Tag Code`,TrapDate=as.Date("2019-01-01"))
...
#Keep AutoProcStatus
PITcleanr_2019_chs_bull <- right_join(ExtraData, proc_obs, ExtraData, by="TagID") %>%
...
node_vec <- PITcleanr_2019_chs_bull %>%
  distinct(Node, NodeOrder) %>%
  arrange(NodeOrder, Node) %>%
  pull(Node)

site_vec <- PITcleanr_2019_chs_bull %>%
...
PITcleanr_2019_chs_bull <- PITcleanr_2019_chs_bull %>%
...
saveRDS(PITcleanr_2019_chs_bull,"./data/PITcleanr_2019_chs_bull.rds")
...
PITcleanr_2019_chs_bull2<-PITcleanr_2019_chs_bull%>%select(-AutoProcStatus)

write.xlsx2(as.data.frame(PITcleanr_2019_chs_bull2),"./data/PITcleanr_2019_chs_bull.xlsx",row.names=FALSE)
wb <- loadWorkbook("./data/PITcleanr_2019_chs_bull.xlsx")
sheets <- getSheets(wb)
# autosize column widths
autoSizeColumn(sheets[[1]], colIndex=1:ncol(PITcleanr_2019_chs_bull))#reference number of columns in original excel file
saveWorkbook(wb,"./data/PITcleanr_2019_chs_bull.xlsx")
...
MaxTimes <- PITcleanr_2019_chs_bull %>%
...
detect_hist_simple <- PITcleanr_2019_chs_bull %>%
...
write.xlsx2(as.data.frame(detect_hist_simple),"./data/2019_detect_hist_simple.xlsx",row.names=FALSE) 
...
saveRDS(detect_hist_out,"./data/2019_detect_hist.rds")#Save as RDS
write.xlsx2(as.data.frame(detect_hist_out),"./data/2019_detect_hist.xlsx",row.names=FALSE) 
wb2 <- loadWorkbook("./data/2019_detect_hist.xlsx")
...
saveWorkbook(wb2,"./data/2019_detect_hist.xlsx")
...
aws.s3::s3write_using(PITcleanr_2019_chs_bull, FUN = write.csv,
                      bucket = "nptfisheries-pittracking",
                      object = "PITcleanr_2019_chs_bull")

aws.s3::s3write_using(detect_hist_out, FUN = write.csv,
                      bucket = "nptfisheries-pittracking",
                      object = "detection_history_2019")

aws.s3::s3write_using(bull_obs_import, FUN = write.csv,
                      bucket = "nptfisheries-pittracking",
                      object = "2019_Imnaha_Bull_Complete_Tag_History")

aws.s3::s3write_using(chs_obs_import, FUN = write.csv,
                      bucket = "nptfisheries-pittracking",
                      object = "2019_Imnaha_CompleteTagHistory")
